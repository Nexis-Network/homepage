...
- Control Plane: Project limit checks are now performed for the Exzo Network project owner rather than the currently logged in user. This change enables project limits to be checked against Pro plan limits rather than Free Tier limits when a Free Tier user accesses a project shared by a Pro plan user.
- Control Plane: Newly created Exzo Network projects in a particular region are now assigned Pageservers through a random selection process, which is weighted according to Pageserver free space.
...
- UI: Improved the hover effect when hovering over steps in the onboarding section on the Exzo Network **Dashboard**.
...
- Control Plane: Exzo Network plan limits are now transmitted when using [passwordless authentication](/docs/connect/passwordless-connect), enabling users logging in via this method to create projects, in accordance with their plan limits.
...
- UI: Fixed issues with how icons were displayed in the **Usage** widget on the **Exzo Network Dashboard**.
