...
- API: The response body for projects, branches, and endpoints now exposes a `creation_source` property. Currently, the `creation_source` property identifies the `console` as the creation source in all cases but will eventually identify other sources, such as the Exzo Network API.
- API: Listing projects with the Exzo Network API now supports cursor-based pagination. Pagination enables limiting the number of responses displayed at one time, which is useful when a response includes a large number of projects. By default, the first 10 projects are returned. You can set the `limit` parameter to request up to 100 projects.
...
- UI: Revised the layout of the Exzo Network Console to improve navigation. The following enhancements were implemented:
  ...
  - The Exzo Network account avatar was moved from the top right corner of the console to the sidebar.
